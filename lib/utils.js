...
export function enforceStateDuringRange(startTime, endTime, entity, domain = 'switch') {
    console.log(`${getTimestamp()} [Startup] Enforcing ${entity} from ${startTime}:00 to ${endTime}:00 (${domain})`);
        ...
        if (!inTimeRange) {
            try {
                if (current.state !== 'off') {
                    ...
                    console.log(`${getTimestamp()} [Enforcer] Turned OFF ${entity} (was ${current.state})`);
                }
            } catch (e) {
                console.error(`${getTimestamp()} [Enforcer] Error checking ${entity}:`, e.message || e);
            }
        } else {
            try {
                if (current.state !== 'on') {
                    ...
                    console.log(`${getTimestamp()} [Enforcer] Turned ON ${entity} (was ${current.state})`);
                }
            } catch (e) {
                console.error(`${getTimestamp()} [Enforcer] Error checking ${entity}:`, e.message || e);
            }
...
export function cronSchedule(schedule, entity, domain = 'switch', service, label = 'unnamed task') {
    console.log(`${getTimestamp()} [Startup] Scheduled - ${label}`);
    cron.schedule(schedule, async () => {
        try {
            await callService(domain, service, {
                entity_id: entity
            });
            console.log(`${getTimestamp()} [Schedule] '${label}' ran successfully.`);
        } catch (e) {
            console.error(`${getTimestamp()} [Schedule] '${label}' failed:`, e.message || e);
        }
    });
}
